...
let countries = ['Finland', 'Sweden', 'Egypt','Denmark', 'Norway','Russia', 'IceLand']
...
countries = ['Finland', 'Sweden', 'Denmark', 'Norway', 'IceLand', 'italy', 'spain', 'germany', 'portugal', 'england', 'netherland', 'belgium', 'france', 'nigeria', 'U.S.A'];

const countryWithPattern = countries.filter(country => {
    country = country.toLowerCase()
    return ( country.includes('land') || country.includes('ia') || country.includes('island')|| country.includes('stan'))
})

console.log(countryWithPattern)


// Better modular approach of above problem
// const categorizeCountries = (countries) => {
//     let coun = countries.filter(country => {
//         country = country.toLowerCase()
//         return country.includes("ia") || country.includes("island") || country.includes("land") || country.includes("stan");
//     })
//     return coun;
// }
...
const getFirstTenCountries = countries => {
    ...
    // let coun = countries.map(country => {
...
for(let i=0 ; i<countries_data.length;i++){
    for (let j=0;j<countries_data[i].languages.length;j++){
        for(let k of topLanguages){
            if (k.languages == countries_data[i].languages[j]){
                k.count += 1
            }
        }
        let val = {languages:countries_data[i].languages[j] , count :1}
        topLanguages.push(val)
    }
}
...
// console.log(mostSpokenLanguages(topLanguages, 10))
